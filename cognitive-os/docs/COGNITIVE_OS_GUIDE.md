...
> **QA snapshot persistente:** 495 pytest passed, 1 skipped, 20 deselected (516 tests recogidos); ruff + ruff format + mypy (108 source files) + frontend lint + frontend build + Compose config + Alembic head + `git diff --check` → todo verde. Fase 37 suma hardening de workers y approvals sobre la base Fase 33: RBAC local explícito, cifrado de payload ejecutable y persistencia configurable de research.
