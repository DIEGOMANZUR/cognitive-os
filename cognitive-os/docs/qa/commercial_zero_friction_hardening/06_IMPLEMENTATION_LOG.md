...
| 2026-05-22 | Docs vigentes actualizados a snapshot real `944 passed`, Playwright `31 passed`, MCP 5/5 y 67 tools | `README.md`, `docs/*`, `scripts/README.md` | `sync_doc_counts --check`, `git diff --check` dentro de full-qa |
...
- `bash scripts/full-qa.sh` -> 944 passed, 1 skipped, 28 deselected; ruff, format, mypy, Alembic, frontend lint/build, sync counts y diff check OK.
...
- `bash scripts/stress-qa.sh` -> 3 runs, cada uno 944 passed, 1 skipped, 28 deselected.
...
## Gates Verdes Post-Gate `5953b40`

- `bash scripts/full-qa.sh` -> 944 passed, 1 skipped, 28 deselected; ruff,
  format, mypy, Alembic, frontend lint/build, sync counts y diff check OK.
- `npx playwright test --reporter=list` -> 31 passed.
- `bash scripts/stress-qa.sh` -> 3 runs, cada uno 944 passed, 1 skipped,
  28 deselected.
- `LIVE_TESTS_ENABLED=1 bash scripts/full-qa-live.sh` -> 8 passed,
  2 warnings MCP deprecations.
- `/system/mcp` -> 5/5 servers connected, 67 tools; inventario paralelo con
  timeout default 30s.
